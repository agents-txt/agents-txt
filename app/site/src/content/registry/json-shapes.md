...
Governs the per-protocol object shapes inside `agents.json` `payments.*` and the top-level array shapes for `mcp[]`, `skills[]`, `a2a[]`, `ucp[]`, `webmcp[]`. The `$schema`, `version`, `standard`, `site`, and `authorization` blocks are fixed by §5.2 of the spec and are not separately registered.
